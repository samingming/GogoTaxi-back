...
// 쿼리에서 boolean 처리용 (true/false, 1/0, yes/no 등 허용)
...
// 참여 요청 스키마 (seatNumber 옵션 + 자동 배정 지원)
...
const matchRoomsSchema = zod_1.z
  ...
  .refine(
    (data) =>
      (!data.radiusKm &&
        data.departureLat === undefined &&
        data.departureLng === undefined) ||
      (data.departureLat !== undefined && data.departureLng !== undefined),
    {
      message:
        "Latitude and longitude are required when radius is provided",
      path: ["radiusKm"],
    }
  );
 ...
 * - closed 방은 건드리지 않음 (첫 번째 버전 로직 유지)
 */
async function refreshRoomStatus(roomId) {
  ...
  if (!snapshot) return;
  if (snapshot.status === client_1.RoomStatus.closed) return;

  const nextStatus =
    snapshot.participants.length >= snapshot.capacity
      ? client_1.RoomStatus.full
      : client_1.RoomStatus.open;
...
async function listRooms(req, res) {
  ...
  const effectiveStatus =
    status ?? (!includeMine && hasSeat ? client_1.RoomStatus.open : undefined);
    ...
    if (
      ...
      sortedRooms = [...rooms].sort((a, b) => {
        const distA = haversineKm(
          refLat,
          refLng,
          a.departureLat.toNumber(),
          a.departureLng.toNumber()
        );
        const distB = haversineKm(
          refLat,
          refLng,
          b.departureLat.toNumber(),
          b.departureLng.toNumber()
        );
        return distA - distB;
      });
    } else if (
      sortBy === "arrivalDistance" &&
      refLat !== undefined &&
      refLng !== undefined
    ) {
      sortedRooms = [...rooms].sort((a, b) => {
        const distA = haversineKm(
          refLat,
          refLng,
          a.arrivalLat.toNumber(),
          a.arrivalLng.toNumber()
        );
        const distB = haversineKm(
          refLat,
          refLng,
          b.arrivalLat.toNumber(),
          b.arrivalLng.toNumber()
        );
        return distA - distB;
      });
    } else if (sortBy === "time") {
...
async function listMyRooms(req, res) {
  ...
  try {
    ...
    return res.json({
      rooms: rooms.map((room) => serializeRoom(room, userId)),
    });
  } catch (error) {
...
async function matchRooms(req, res) {
  ...
  if (departureLat !== undefined && departureLng !== undefined) {
    ...
    const lngDelta =
      radiusKm /
      (111 * Math.max(Math.cos((departureLat * Math.PI) / 180), 0.0001));
  ...
  try {
    ...
    return res.json({
      rooms: filtered.map((room) => serializeRoom(room, userId)),
    });
  } catch (error) {
...
async function updateRoom(req, res) {
  ...
  try {
    ...
    if (room.status !== client_1.RoomStatus.open) {
      return res
        .status(400)
        .json({ message: "Only open rooms can be updated" });
    }
...
/**
 * 방 참여 (좌석 선택, seatNumber 없으면 자동 배정)
 */
async function joinRoom(req, res) {
  ...
  try {
    ...
    if (room.status !== client_1.RoomStatus.open) {
      return res
        .status(400)
        .json({ message: "Room is not open for joining" });
    }
    ...
    if (seatNumber > room.capacity) {
      return res
        .status(400)
        .json({ message: "Seat number exceeds room capacity" });
    }
...
async function leaveRoom(req, res) {
  ...
  try {
    ...
    // 방장인 경우 처리 (두 번째 버전 로직 유지)
    if (room.creatorId === userId) {
      if (room.participants.length > 1) {
        return res.status(403).json({
          message: "Host cannot leave while other participants remain",
        });
      }

      await prisma_1.prisma.$transaction([
        prisma_1.prisma.roomParticipant.deleteMany({
    ...
    if (!participant) {
      return res
        .status(404)
        .json({ message: "Not participating in this room" });
    }

    await prisma_1.prisma.roomParticipant.delete({
    ...
    return res.json({
      room: updated ? serializeRoom(updated, userId) : null,
    });
  } catch (error) {
