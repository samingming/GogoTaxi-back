...
const corsOptions = {
    origin(origin, callback) {
        ...
        callback(new Error(`Origin ${origin} is not allowed by CORS (configure ALLOWED_ORIGINS).`));
    },
    credentials: true,
...
const fetchGoogleProfile = async (token) => {
    ...
    if (!response.ok) {
        const errorText = await response.text();
        throw new Error(`Google token verification error: ${response.status} ${errorText}`);
    }
    const data = (await response.json());
...
const ensureUserForSocialProfile = async (profile) => {
